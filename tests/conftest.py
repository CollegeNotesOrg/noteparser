...
def sample_documents(temp_dir):
    ...
    md_doc.write_text("""# AI Research Notes
    ...
    return {
        'ragflow': {
            ...
            'insights': {
                ...
                'keywords': ['machine learning', 'neural networks', 'AI', 'deep learning', 'algorithms'],
...
def pytest_configure(config):
    ...
    config.addinivalue_line(
        "markers", "benchmark: mark test as benchmark/performance test"
    )
    config.addinivalue_line(
...
def pytest_collection_modifyitems(config, items):
    ...
    for item in items:
        ...
        if any(keyword in item.nodeid.lower() for keyword in ["large", "batch", "concurrent", "load"]):
...
def pytest_runtest_setup(item):
    ...
    if "integration" in item.keywords:
        if not os.getenv("INTEGRATION_TESTS", "").lower() in ("1", "true", "yes"):
    ...
    if "benchmark" in item.keywords:
        if not os.getenv("BENCHMARK_TESTS", "").lower() in ("1", "true", "yes"):
...
def clean_environment():
    ...
    original_env = {}
    ai_env_vars = [
        'RAGFLOW_URL', 'DEEPWIKI_URL', 'AI_SERVICES_ENABLED',
